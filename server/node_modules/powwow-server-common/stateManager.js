...
exports.startRequest = function(page) {
    requestInProgress = true;
    if(config.onRequest) {
        config.onRequest.call(config, page);
    }
};
...
function updateDataSubtree(data, newDataMap, prefix) {
    var keys = Object.keys(data);
    for(var i=0; i < keys.length; i++) {
      var key = keys[i];
      var path = prefix ? prefix + '.' + key : key;
      if(newDataMap.hasOwnProperty(path)) {
        data[key] = newDataMap[path];
      } else if(_.isArray(data[key])) {
        for(var j=0; j < data[key].length; j++) {
          updateDataSubtree(data[key][j], newDataMap, path + '[' + j + ']');
        }
      } else if(_.isObject(data[key])) {
        updateDataSubtree(data[key], newDataMap, path);
      }
    }
}
// Update the current screen data.
exports.updateCurrentScreenData = function(newDataMap) {
    updateDataSubtree(currentScreen.params, newDataMap, "");
};
...
exports.onconnection = function(connection) {
    ...
    // fixes issue with reconnect - when one user connects,
    // we disconnect other user, and disconnect is called later and teardownTimeout starts
    // but the user is actually connected
    setTimeout(function() {
        if (teardownTimeout) {
            logger.info('teardownTimeout cleared onconnection');
...
exports.initializeConnection = function() {
    ...
    // Send initialization data to the client if we have any.
    if(config.getInitializationData) {
        client.initialize(config.getInitializationData());
    }

    if (teardownTimeout) {
        logger.info('teardownTimeout cleared initializeConnection');
        clearTimeout(teardownTimeout);
        teardownTimeout = null;
    }
    if (currentScreen.name) {
        ...
        if(config.hasOwnProperty('startURL')) {
            browser.createBrowserInstanceAndNewPage(proxyInfo, exports.connectToPage);
            stateDetector.register();
        } else if(config.hasOwnProperty('startMethod')) {
            var api = require.main.require('./api');
            logger.info("New session starting with method:", config.startMethod);
            var arrStartMethodParts = config.startMethod.split(".");
            var namespace = arrStartMethodParts[0];
            var action = arrStartMethodParts[1];

            if (typeof api[namespace] === 'object' && typeof api[namespace][action] === 'function') {
                logger.info("Found ", namespace + "." + action);
                try {
                    api[namespace][action].call(api[namespace]);
                } catch (e) {
                    logger.error(e.message);
                }
            } else {
                logger.info('This startMethod does not exist: ' + namespace + "." + action);
            }
        } else {
            logger.error("Please specify either startURL or startMethod in config.js");
        }
    }
};
                ...
                if(window.powwow) {
                    ...
                    function copyOverPageModelIdentifiersAndResolveSubDescriptors(node, nodetoCopyTo, id, mapPageModels, nodeParent) {
                        if(node.type == "object") {
                            ...
                            // Recurse over the properties to look for identifiers.
                            copyOverPageModelIdentifiersAndResolveSubDescriptors(node.items, nodetoCopyTo[id].items, null, mapPageModels, node.items);
                            ...
                            nodetoCopyTo[id] = {};
                            copyOverPageModelIdentifiersAndResolveSubDescriptors(node, nodetoCopyTo[id], null, mapPageModels, node);
                        }
                    }
...
exports.onLoadFinished = function(pageInfo) {
    ...
    function createResourceLoadCallback(proc) {
        return function() {
            logger.info('Matched registered url or resource: \'' + proc.name + '\'');
    ...
    for (var i = 0; i < loadFinishedProcessors.length; i++) {
        ...
        if (matchResult === MATCHTYPE.MATCH) {
            if (proc.resourcePattern) {
                ...
                logger.info('Matched registered url or resource: \'' + loadFinishedProcessors[i].name + '\'');
