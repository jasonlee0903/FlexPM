...
if(system.args.length > 1) {
  userAgentOverride = system.args[1];
}
var pages  = {};
var page_id = 1;
...
function page_open (res, page, args) {
  page.open.apply(page, args.concat(function (success) {
    res.statusCode = 200;
    var body = JSON.stringify({ data: success });
    res.setHeader('Cache', 'no-cache');
    res.setHeader('Content-Type', 'application/json');
    res.setHeader('Connection', 'Keep-Alive');
    res.setHeader('Keep-Alive', 'timeout=5, max=100')
    res.setHeader('Content-Length', getByteLen(body));
    res.write(body);
    res.close();
  }));
}
...
webserver.listen('127.0.0.1:0', { keepAlive: true }, function (req, res) {
  // Update watchdog timer on every request
  watchdog_clear();

  if (req.method === 'GET') {
    res.statusCode = 200;
    var body = JSON.stringify({ data: callback_stack });
    res.setHeader('Cache', 'no-cache');
    res.setHeader('Content-Type', 'application/json');
    res.setHeader('Connection', 'Keep-Alive');
    res.setHeader('Keep-Alive', 'timeout=5, max=100')
...
function setup_callbacks (id, page) {
  ...
  page.onPageCreated = function (page) {
    if(userAgentOverride) {
      page.customHeaders = { 'User-Agent': userAgentOverride };
    }
